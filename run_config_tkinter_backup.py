...
# PyQt5 imports for file dialogs (fallback to tkinter for complex GUI)
try:
    ...
    def askopenfile(**kwargs):
        """PyQt5 replacement for tkinter.filedialog.askopenfile"""
        app = QApplication.instance()
        if app is None:
            app = QApplication(sys.argv)
        ...
        if filename:
            return open(filename, kwargs.get('mode', 'r'))
        return None

except ImportError:
    PYQT5_AVAILABLE = False
    # Fallback to tkinter for file dialogs
    from tkinter.filedialog import asksaveasfile, askopenfile

# Import tkinter for the complex GUI (will be converted later if needed)
